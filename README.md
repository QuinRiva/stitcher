...
`stitcher` is a small Python library for **single-schema** structured
...
(`existing={id1: ..., id2: ...}` keyed by `tool_call_id`); `stitcher`
...
from stitcher import Extractor, AggregatedValidationError
...
| Use case | Tool |
|---|---|
| Single Pydantic schema, single response, JSON-Patch repair | **stitcher** (`ainvoke`) |
| Update a single existing instance per a user intent | **stitcher** (`aupdate`) |
...
`stitcher` does the same job with less surface area and avoids the
...
stitcher = { git = "https://github.com/QuinRiva/stitcher.git", rev = "<sha>" }
```
